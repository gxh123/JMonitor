...
/**
 * Created by jmonitor on 2016/12/14.
 */
public class EventReportService  extends AbstractReportService<EventReport> {
    @Override
    public EventReport query(Date startTime, String tableName){
        String sql = "SELECT * FROM " + tableName +" WHERE startTime = ? and reportType = ?";
        List<Record> records = Db.find(sql, startTime, "Event");
        if(records.size() != 0){
            String content = new String((byte[])records.get(0).get("content"));
            EventReport report = JsonUtil.fromJson(content, EventReport.class);
            return report;
        }else{
            return null;
        }
    ...
    @Override
    protected EventReport sendQueryAndWait(Date startTime, String step) {
        String prefix = "QUERY";
        String command = prefix + "?time=" + new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(startTime)
                + "&step=" + step + "&type=" + "Event" + "EOF\n";
        return sendQueryAndWaitInternal(command, new EventReport());
    }
